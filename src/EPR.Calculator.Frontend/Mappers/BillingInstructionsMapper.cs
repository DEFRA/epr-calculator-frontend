...
{
    /// <summary>
    ///     Maps a <see cref="ProducerBillingInstructionsResponseDto" /> and pagination request to a
    ///     <see cref="BillingInstructionsViewModel" />.
    /// </summary>
    ...
    BillingInstructionsViewModel MapToViewModel(
        ...
        BillingInstructionsIndexModel request,
        bool isSelectAll,
...
public class BillingInstructionsMapper : IBillingInstructionsMapper
{
        ...
        bool isSelectAllPage)
    {
        ...
        return new BillingInstructionsViewModel
        {
            ...
            RunName              = billingData?.RunName ?? string.Empty,
            ...
            TablePaginationModel = new PaginationViewModel
            {
                ...
                {
                    ["runId"]                                                   = billingData?.CalculatorRunId ?? 0,
                    [nameof(BillingInstructionsIndexModel.PageSize)]            = request.PageSize,
                    [nameof(BillingInstructionsIndexModel.OrganisationId)]      = request.OrganisationId,
                    [nameof(BillingInstructionsIndexModel.BillingInstructions)] = request.BillingInstructions.ToArray(),
                    [nameof(BillingInstructionsIndexModel.BillingStatuses)]     = request.BillingStatuses.ToArray()
                }
            },
    ...
    private static BillingInstruction MapBillingInstruction(string suggested)
    {
        ...
        if (Enum.TryParse<BillingInstruction>(normalized, true, out var result))
            return result;
    ...
    private static BillingStatus MapBillingStatus(string? acceptReject)
    {
        ...
        if (Enum.TryParse<BillingStatus>(normalized, true, out var result))
            return result;
